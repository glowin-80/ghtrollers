...
export default function MinSidaPage() {
  ...
  const visibleCatches = useMemo(
    () =>
      catches.filter(
        (item) =>
          (showLiveScope || !item.live_scope) &&
          (showAbroad || !item.caught_abroad)
      ),
    [catches, showAbroad, showLiveScope]
  );

  const selectedCatch = useMemo(
    () => visibleCatches.find((item) => item.id === selectedCatchId) || null,
    [visibleCatches, selectedCatchId]
  );
        ...
        <div className="mx-auto max-w-5xl space-y-4">
          ...
          {catchesError ? (
            <section className="rounded-[26px] border border-amber-200 bg-white/95 p-5 text-[#7a4b00] shadow-[0_8px_24px_rgba(18,35,28,0.06)]">
              ...
              <button
                type="button"
                onClick={() => loadMemberCatches(member)}
            ...
            <>
              <section className="rounded-[28px] border border-[#d8d2c7] bg-white/95 p-5 shadow-[0_8px_24px_rgba(18,35,28,0.06)]">
                <div className="text-[0.78rem] font-semibold uppercase tracking-[0.14em] text-[#7a6540]">
                  Filter på mina sidor
                </div>

                <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                  <label className="flex items-start gap-3 rounded-2xl border border-[#e5ddd0] bg-[#fffdf9] px-4 py-3 text-sm text-[#374151]">
                    <input
                      type="checkbox"
                      checked={showLiveScope}
                      onChange={(e) => setShowLiveScope(e.target.checked)}
                      className="mt-1 h-4 w-4 rounded border-[#cfc6b8] text-[#324b2f] focus:ring-[#d9cfbf]"
                    />
                    <span>Visa fångster med Live-scope</span>
                  </label>
                  ...
                  <label className="flex items-start gap-3 rounded-2xl border border-[#e5ddd0] bg-[#fffdf9] px-4 py-3 text-sm text-[#374151]">
                    <input
                      type="checkbox"
                      checked={showAbroad}
                      onChange={(e) => setShowAbroad(e.target.checked)}
                      className="mt-1 h-4 w-4 rounded border-[#cfc6b8] text-[#324b2f] focus:ring-[#d9cfbf]"
                    />
                    <span>Visa mina fångster utomlands</span>
                  </label>
                </div>
              </section>
              ...
              <StatsGrid
                ...
                onSelectCatch={setSelectedCatchId}
              />
