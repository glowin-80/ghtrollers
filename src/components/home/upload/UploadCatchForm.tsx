...
export default function UploadCatchForm({ members, caughtFor, registeredBy, fishType, fineFishType, weight, catchDate, fishingMethod, caughtAbroad, isLocationPrivate, locationName, latitude, longitude, gpsLoading, gpsError, formMessage, previewUrl, fileInputKey, loading, onSubmit, onCaughtForChange, onFishTypeChange, onFineFishTypeChange, onWeightChange, onCatchDateChange, onFishingMethodChange, onCaughtAbroadChange, onIsLocationPrivateChange, onOpenLocationChooser, onGetGps, onOpenMap, onImageChange, onDismissFormMessage, onFormMessageAction }: UploadCatchFormProps) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="rounded-2xl border border-[#d8d2c7] bg-[#fffdfb] px-4 py-4 text-sm text-[#374151]">
        <div className="text-xs font-semibold uppercase tracking-[0.12em] text-[#6b7280]">Tävlingsregel</div>
        <select value={fishingMethod} onChange={(e) => onFishingMethodChange(e.target.value)} className="mt-3 w-full rounded-2xl border border-[#d8d2c7] bg-white px-4 py-3 text-[#1f2937] outline-none transition focus:border-[#8b7b68] focus:ring-2 focus:ring-[#d9cfbf]" required><option value="">Välj fiskemetod</option><option value="Trolling">Trolling</option><option value="Spinnfiske">Spinnfiske</option><option value="Metspö">Metspö</option><option value="Live-scope">Live-scope</option></select>
        <label className="mt-3 flex items-start gap-3 rounded-2xl border border-[#e5ddd0] bg-white px-4 py-3"><input type="checkbox" checked={caughtAbroad} onChange={(e) => onCaughtAbroadChange(e.target.checked)} className="mt-1 h-4 w-4 rounded border-[#cfc6b8] text-[#324b2f] focus:ring-[#d9cfbf]" /><span><span className="block font-semibold text-[#1f2937]">Utomlands</span><span className="mt-1 block text-xs leading-5 text-[#6b7280]">Fångster med Live-scope eller markerade som utomlands räknas inte i tävlingen.</span></span></label>
      </div>
