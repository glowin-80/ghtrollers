...
export default function UploadCatchForm({
  ...
  onWeightChange,
  onCatchDateChange,
  onFishingMethodChange,
  onLiveScopeChange,
  onCaughtAbroadChange,
  onIsLocationPrivateChange,
  ...
  return (
    <form onSubmit={onSubmit} noValidate className="space-y-4">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <label className="flex items-start gap-3 rounded-2xl border border-[#e5ddd0] bg-white px-4 py-3">
            <input
              type="checkbox"
              checked={liveScope}
              onChange={(e) => onLiveScopeChange(e.target.checked)}
            ...
            <span>
              ...
              <span className="mt-1 block text-xs leading-5 text-[#6b7280]">
                Fångster med Live-scope räknas inte i tävlingen.
              </span>
            </span>
          </label>
          ...
          <label className="flex items-start gap-3 rounded-2xl border border-[#e5ddd0] bg-white px-4 py-3">
            <input
              type="checkbox"
              checked={caughtAbroad}
              onChange={(e) => onCaughtAbroadChange(e.target.checked)}
              className="mt-1 h-4 w-4 rounded border-[#cfc6b8] text-[#324b2f] focus:ring-[#d9cfbf]"
            />
            <span>
              <span className="block font-semibold text-[#1f2937]">
              ...
              <span className="mt-1 block text-xs leading-5 text-[#6b7280]">
                Fångster utomlands räknas inte i tävlingen.
              </span>
            </span>
          </label>
        </div>
      ...
      <div className="rounded-2xl border border-[#d8d2c7] bg-[#fffdfb] px-4 py-3 text-sm">
        ...
        <div className={locationSummaryTextClass}>Latitud: {latitude ?? "Saknas"}</div>
        <div className={locationSummaryTextClass}>Longitud: {longitude ?? "Saknas"}</div>

        <label className="mt-3 flex items-start gap-3 rounded-2xl border border-[#e5ddd0] bg-white px-4 py-3">
          <input
            type="checkbox"
            checked={isLocationPrivate}
            onChange={(e) => onIsLocationPrivateChange(e.target.checked)}
            className="mt-1 h-4 w-4 rounded border-[#cfc6b8] text-[#324b2f] focus:ring-[#d9cfbf]"
          />
          <span>
            <span className="block font-semibold text-[#1f2937]">
            ...
            <span className="mt-1 block text-xs leading-5 text-[#6b7280]">
              Privat plats syns bara för dig själv.
            </span>
          </span>
        </label>
      </div>
