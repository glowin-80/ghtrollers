  ...
  const summaries = useMemo(
    () =>
      CATCH_REACTION_EMOJIS.map((emoji) => {
        return (
    <div className={`${compact ? "" : ""} relative inline-flex items-center gap-1.5`}>
          ...
          {visibleSummaries.map((summary) => {
            ...
            return (
              <div key={summary.emoji} className="relative">
                ...
                {namesOpen ? (
                  <div className="absolute right-0 top-9 z-30 min-w-40 max-w-56 rounded-2xl border border-[#d8d2c7] bg-white px-3 py-2 text-xs font-semibold text-[#31414b] shadow-[0_8px_24px_rgba(18,35,28,0.16)]">
                    {reactedByText}
                  </div>
                ) : null}
              </div>
