...
export default function CatchReactionBar({
  ...
  const summaries = useMemo<CatchReactionSummary[]>(() => {
  ...
  return (
    <div className={`${compact ? "" : ""} relative inline-flex flex-wrap items-center gap-1.5`}>
      {visibleSummaries.map((summary) => {
        ...
        return (
          <div key={summary.emoji} className="relative">
            ...
            {namesOpen ? (
              <div className="absolute bottom-10 right-0 z-30 min-w-40 max-w-56 rounded-2xl border border-[#d8d2c7] bg-white px-3 py-2 text-xs font-semibold text-[#31414b] shadow-[0_8px_24px_rgba(18,35,28,0.16)]">
                {reactedByText}
              </div>
            ) : null}
          </div>
      ...
      <div className="relative">
        <button
          ...
          className={`flex h-9 w-9 items-center justify-center rounded-full border border-[#d8d2c7] bg-white text-lg shadow-sm transition hover:bg-[#f7f4ee] ${
        ...
        {pickerOpen ? (
          <div className="absolute bottom-11 right-0 z-30 flex items-center gap-1 rounded-full border border-[#d8d2c7] bg-white p-1.5 shadow-[0_8px_24px_rgba(18,35,28,0.16)]">
