        ...
        >
          ...
          <div className="relative overflow-y-auto px-4 pb-4 pt-4 sm:px-4 sm:pb-4">
            ...
            {catchItem.image_url ? (
              <button
                type="button"
                onClick={() => setSelectedImage(catchItem.image_url || null)}
                className="mt-3.5 block w-full overflow-hidden rounded-[18px] border border-[#e5ddd0] bg-[#fffdf9] shadow-[0_8px_18px_rgba(18,35,28,0.06)] transition hover:scale-[1.01]"
                aria-label={`Öppna större bild för ${displayFishName}`}
              >
                <img
        ...
        >
          ...
          >
            <button
              type="button"
              onClick={() => setSelectedImage(null)}
              className="absolute right-3 top-3 z-20 inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/30 bg-black/60 text-[1.25rem] font-bold leading-none text-white shadow-[0_6px_16px_rgba(0,0,0,0.3)] transition hover:bg-black/75"
